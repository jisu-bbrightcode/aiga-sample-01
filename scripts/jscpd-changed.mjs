...
const supportedExtension = /\.(?:ts|tsx|js|jsx|css)$/;
const supportedRoot = /^(?:apps|packages|tools)\//;
...
function isSupportedFile(file) {
  ...
  return supportedRoot.test(repoRelativePath) && supportedExtension.test(repoRelativePath);
}
...
const files = [...new Set([...changed, ...untracked])]
  ...
  .filter((file) => existsSync(file) && isInsideScope(file) && isSupportedFile(file))
  .map((file) => relative(process.cwd(), file) || ".");

if (files.length === 0) {
  process.stdout.write("No changed files matched the jscpd scope.\n");
  process.exit(0);
}

const result = spawnSync("jscpd", ["--config", resolve(repoRoot, ".jscpd.json"), ...files], {
  stdio: "inherit",
});
