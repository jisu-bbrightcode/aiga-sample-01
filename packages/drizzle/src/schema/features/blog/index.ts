...
export const blogClaps = pgTable(
  ...
  (table) => [uniqueIndex("blog_claps_unique").on(table.postId, table.userId)],
);
 ...
 */
export const blogResponses = pgTable("blog_responses", {
  ...baseColumns(),
...
export const blogBookmarks = pgTable(
  ...
  (table) => [uniqueIndex("blog_bookmarks_unique").on(table.userId, table.postId)],
);
