...
async function createCommentTable() {
  ...
  // Create indexes
  await sql.unsafe(
    "CREATE INDEX IF NOT EXISTS idx_comment_comments_target ON comment_comments(target_type, target_id);",
  );
  await sql.unsafe(
    "CREATE INDEX IF NOT EXISTS idx_comment_comments_parent ON comment_comments(parent_id);",
  );
  await sql.unsafe(
    "CREATE INDEX IF NOT EXISTS idx_comment_comments_author ON comment_comments(author_id);",
  );
  await sql.end();
}
