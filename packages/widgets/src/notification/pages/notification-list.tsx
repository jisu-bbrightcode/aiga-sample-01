...
import type { NotificationItem as NotificationItemType } from "../../common/types";
...
export function NotificationList({
  ...
  if (items.length === 0) {
    return <div className="py-8 text-center text-sm text-muted-foreground">알림이 없습니다</div>;
  }
    ...
    <div className="flex flex-col">
      {hasUnread && (
        <div className="flex justify-end border-b px-3 py-2">
          ...
          >
            {markAllAsRead.isPending ? <Loader2 className="mr-2 size-3.5 animate-spin" /> : null}
            전체 읽음
          </Button>
        </div>
      )}
