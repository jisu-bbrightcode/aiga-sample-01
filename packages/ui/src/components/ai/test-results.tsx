...
export const TestSuiteName = ({ className, children, ...props }: TestSuiteNameProps) => {
  ...
  return (
    <CollapsibleTrigger
      className={cn(
        "group flex w-full items-center gap-2 px-4 py-3 text-left transition-colors hover:bg-muted",
        className,
      )}
      {...props}
    >
...
const statusIcons: Record<TestStatus, React.ReactNode> = {
  passed: <CheckCircle2Icon className="size-3.5" />,
  failed: <XCircleIcon className="size-3.5" />,
  skipped: <CircleIcon className="size-3.5" />,
  running: <CircleDotIcon className="size-3.5 animate-pulse" />,
};
