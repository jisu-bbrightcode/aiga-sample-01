...
import { Dialog, DialogContent, DialogTitle, DialogTrigger } from "@repo/ui/shadcn/dialog";
import {
  CircleIcon,
  LoaderCircleIcon,
  MarsIcon,
...
export const VoiceSelector = ({
  ...
  const [open, setOpen] = useControllableState({
    ...
    onChange: (nextOpen) => onOpenChange?.(nextOpen, undefined as never),
  });
...
export const VoiceSelectorGender = ({
  ...
  switch (value) {
    case "male":
      icon = <MarsIcon className="size-3.5" />;
      break;
    case "female":
      icon = <VenusIcon className="size-3.5" />;
      break;
    default:
...
export const VoiceSelectorPreview = ({
  ...
  let icon = <PlayIcon className="size-3.5" />;
