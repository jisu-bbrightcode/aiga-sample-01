...
export function DayTimeline<T extends TimelineEvent>({
  ...
  useEffect(() => {
    ...
    if (todayIdx >= 0) {
      const scrollX = TIME_COL_WIDTH + todayIdx * dayWidth - el.clientWidth / 2 + dayWidth / 2;
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (scrollLeft + clientWidth >= scrollWidth - SCROLL_THRESHOLD && !loadMoreRef.current) {
          ...
          >
            ...
            <div className="flex" style={{ height: totalHoursHeight, marginTop: BODY_TOP_OFFSET }}>
              ...
              {dates.map((date) => {
                const dateStr = toDateString(date);
                const isToday = dateStr === todayStr;
                const dayEvents = isLoading ? [] : (eventsByDate.get(dateStr) ?? []);
                ...
                return (
                  ...
                  >
                    ...
                    {dayEvents.map((event) => {
                      ...
                      const bottomY = Math.min(totalHoursHeight, getTimeY(event.endTime));
