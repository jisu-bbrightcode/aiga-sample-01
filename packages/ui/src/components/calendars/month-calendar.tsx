  ...
  const calendarDays = useMemo(() => {
    ...
    for (let d = 1; d <= lastDay.getDate(); d++) {
      ...
      const dayEvents = events.filter((e) => {
        const sd =
          typeof e.date === "string" ? e.date.slice(0, 10) : e.date.toISOString().slice(0, 10);
        return sd === dateStr;
      });
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-md px-3 h-8 text-sm font-medium hover:bg-muted transition-colors"
          >
            오늘
          </button>
          <button
          ...
          return (
            <div
              ...
              className={cn(
                ...
                day.date !== null && "hover:bg-muted cursor-pointer",
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-7 gap-px rounded-lg overflow-hidden">
        {Array.from({ length: 35 }).map((_, i) => (
          <div key={i} className="min-h-[80px] lg:min-h-[100px] bg-muted animate-pulse" />
        ))}
      </div>
    </div>
  );
