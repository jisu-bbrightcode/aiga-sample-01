...
export const NotionCalendarTimeline = React.forwardRef<HTMLDivElement, NotionCalendarTimelineProps>(
  (
    ...
    const getTimeFromY = React.useCallback(
      (y: number): string => {
        const totalHours = y / hourHeight + startHour;
    ...
    const [draggedData, setDraggedData] = React.useState<{
      topY: number;
      bottomY: number;
      date: string;
    } | null>(null);
    ...
    React.useEffect(() => {
      ...
      const handlePointerMove = (e: PointerEvent) => {
        ...
        const _deltaX = e.clientX - dragState.initialX;
    ...
    }, [dragState, draggedData, events, hourHeight, totalHoursHeight, onEventChange, getTimeFromY]);

    return (
      <div ref={localRef} className={cn("flex-1 relative select-none", className)} {...props}>
        <div
          ...
          >
            {extraTzWidth > 0 && (
              <div
                ...
                className="shrink-0 border-r border-border/40 bg-muted/5 font-medium"
              />
            )}
            {timezones.map((tz, idx) => (
              ...
              >
                ...
                {tz.offset === 0 && isCurrentTimeVisible && currentTimeY !== null && (
                  <div
                    className="absolute left-1 right-1 -mt-[11px] z-20 flex flex-col items-center justify-center bg-[#eb5757] rounded-[3px] text-white text-2xs font-medium py-0.5 leading-none shadow-sm"
                    style={{ top: currentTimeY }}
                  >
                    <span>
                      {now.getHours() % 12 || 12}:{String(now.getMinutes()).padStart(2, "0")}
                    </span>
                    <span className="text-2xs -mt-[1px] font-normal opacity-90">
                ...
                {hours.map((hour) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="absolute -top-2 right-1.5 text-2xs text-muted-foreground/60 w-full text-right pl-1 pr-1">
                        {displayHour === 0
                          ? "12 AM"
                          : displayHour < 12
                            ? `${displayHour} AM`
            ...
            {days.map((day) => {
              ...
              return (
                <div
                  ...
                  className={cn("flex flex-col border-r border-border/30 relative shrink-0")}
                ...
                >
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          onEmptySlotClick && "cursor-pointer hover:bg-muted transition-colors",
                        )}
                        style={{ height: hourHeight }}
                  ...
                  */}
                  ...
                  {dayEvents.map((event) => {
                    ...
                    const topY = Math.max(0, getTimeY(startT));
                    const bottomY = (event as any).endTime
                      ? Math.min(totalHoursHeight, getTimeY(endT))
                      : topY + hourHeight;
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          "absolute z-[3] rounded-sm overflow-hidden shadow-sm flex text-xs font-medium cursor-pointer transition-transform touch-none border border-transparent hover:opacity-90",
                      ...
                      >
                        <div className="truncate text-foreground text-xs leading-tight">
                          {event.title}
                        </div>
                        {!isShort && (
                          <div className="truncate text-muted-foreground text-2xs leading-none mt-0.5">
                            {isDragged && draggedData
                              ? `${getTimeFromY(finalTopY)} - ${getTimeFromY(finalBottomY)}`
                              : `${startT} - ${endT}`}
                          </div>
                        )}
                        {isShort && (
                          <div className="truncate text-muted-foreground text-2xs leading-none ml-1 shrink-0">
