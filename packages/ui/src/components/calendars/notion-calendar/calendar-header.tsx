...
import { type TimezoneInfo, TimezonePicker } from "./timezone-picker";
      ...
      >
        ...
        >
          ...
          {onAddTimezone && (
            <button
              ...
              className="flex items-center justify-center w-6 h-full text-muted-foreground/40 hover:text-foreground hover:bg-muted transition-colors shrink-0 cursor-pointer"
              title="세계시간 추가"
            >
          ...
          {timezones.map((tz, idx) => (
            <div
              key={`${tz.label}-${idx}`}
              className={cn(
                "flex-1 flex items-center justify-center h-full text-2xs text-muted-foreground/70 relative group cursor-pointer hover:bg-muted transition-colors",
        ...
        <div className="flex flex-1">
          {days.map((day) => (
            <div
              key={day.date}
              className="flex items-center justify-center border-r border-border/40 shrink-0 gap-1.5 cursor-pointer hover:bg-muted transition-colors"
              style={{ width: colWidth, minWidth: "120px" }}
            >
              <span
                className={cn(
                  "text-xs font-normal leading-none",
                  day.isHoliday || day.dayOfWeek === "일"
                    ? "text-red-500/70"
                    : day.dayOfWeek === "토"
                      ? "text-blue-500/70"
              ...
              <div
                className={cn(
                  "flex items-center justify-center h-[22px] min-w-[22px] rounded-full text-base font-medium tabular-nums px-0.5",
              ...
              {/* Holiday badge */}
              {day.holidayName && (
                <span className="text-2xs text-muted-foreground/60 truncate max-w-[60px]">
                  {day.holidayName}
                </span>
              )}
            </div>
