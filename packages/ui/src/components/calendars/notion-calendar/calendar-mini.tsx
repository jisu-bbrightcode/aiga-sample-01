...
export const NotionCalendarMini = React.forwardRef<HTMLDivElement, NotionCalendarMiniProps>(
  (
    ...
    return (
      <div ref={ref} className={cn("w-full select-none text-xs", className)} {...props}>
        {/* Header */}
        <div className="flex items-center justify-between mb-1.5 pl-1 pr-0.5">
          <span className="font-semibold text-foreground text-xs">{currentMonthText}</span>
        ...
        {/* Weekdays */}
        <div className="grid grid-cols-7 mb-0.5 text-center text-muted-foreground/70 text-2xs font-medium">
          {weekdays.map((day) => (
            ...
            days.map((day, i) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "relative flex items-center justify-center h-6 w-6 mx-auto rounded-full transition-colors text-xs",
                  ...
                  style={
                    isInView && !day.isToday
                      ? { backgroundColor: "rgba(0,0,0,0.08)", borderRadius: "4px" }
                      : undefined
                  }
                >
                  {day.date}
