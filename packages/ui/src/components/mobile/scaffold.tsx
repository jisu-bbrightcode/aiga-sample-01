  ...
  return (
    <div className={cn("flex h-14 shrink-0 gap-x-4 px-3", className)}>
      <div className="flex shrink-0 items-center gap-x-2">
        ...
        {leftActions.map(({ icon, label, onClick }) => (
          <Button key={`left-${icon}-${label}`} variant="ghost" size="icon" onClick={onClick}>
      ...
      <div className="flex shrink-0 items-center gap-x-2">
        {rightActions.map(({ icon, label, onClick }) => (
          <Button key={`right-${icon}-${label}`} variant="ghost" size="icon" onClick={onClick}>
            <DynamicIcon name={icon} className="size-6" />
            <span className="sr-only">{label}</span>
          </Button>
        ))}
...
export function ScaffoldContent({ children, className }: ScaffoldContentProps) {
  return (
    <PullToRefresh className={cn("min-h-0 flex-1 basis-0", className)}>{children}</PullToRefresh>
  );
}
