    ...
    return (
      <div className={cn("flex justify-end pr-2", className)}>
        <div className="max-w-[85%] rounded-[1.25rem] rounded-tr-sm bg-primary/10 text-primary-foreground/90 px-4 py-2.5 shadow-sm ring-1 ring-primary/20">
          <span className="whitespace-pre-wrap text-lg leading-relaxed font-medium">
            {content}
          </span>
        </div>
      </div>
      ...
      {showAvatar && (
        <Avatar className="mt-1 size-8 shrink-0 shadow-sm border border-border/50">
          <AvatarFallback className="bg-primary/5 text-primary">
            {avatarIcon ?? <Bot className="size-3.5" />}
          </AvatarFallback>
        </Avatar>
      )}
    ...
    <Markdown
      ...
      components={{
        ...
        code: ({ className, children }) => {
          ...
          return (
            <code className="rounded-md bg-muted px-1.5 py-0.5 text-base font-mono">
              {children}
            </code>
          );
        },
...
function CodeBlock({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="my-3 overflow-hidden rounded-lg border border-border/50">
      ...
      <pre className="overflow-x-auto bg-muted/30 p-4 text-base leading-relaxed">{children}</pre>
    </div>
  );
}
