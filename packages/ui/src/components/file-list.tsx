 ...
 * @example
 * import { FileList } from "@repo/ui/components/file-list";
 * <FileList files={files} view="grid" onDelete={handleDelete} />
 */
import { Download, FileIcon, FileText, Film, Image, Music, Trash2 } from "lucide-react";
import { Button } from "../_shadcn/button";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "../_shadcn/table";
...
export function FileList({
  ...
  if (isLoading) {
    return (
      <div className="text-muted-foreground flex items-center justify-center py-8">로딩 중...</div>
    );
  }
                ...
                <div className="flex justify-end gap-2">
                  {file.publicUrl && (
                    <a
                      ...
                      className="inline-flex size-8 items-center justify-center rounded-md border border-input bg-background hover:bg-muted hover:text-foreground"
                    >
...
function getFileIcon(mimeType: string) {
  if (mimeType.startsWith("image/")) return <Image className="size-3.5" />;
  if (mimeType.startsWith("video/")) return <Film className="size-3.5" />;
  if (mimeType.startsWith("audio/")) return <Music className="size-3.5" />;
