...
function statusToLogLevel(status: number): "error" | "warn" | "info" {
...
export function otelLogger(): MiddlewareHandler {
  ...
  return async (c, next) => {
    ...
    const level = statusToLogLevel(status);
    ...
    logger[level](status >= 400 ? "Request failed" : "Request completed", attributes);
  };
}
