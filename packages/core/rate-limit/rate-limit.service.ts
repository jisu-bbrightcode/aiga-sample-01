...
import { DRIZZLE, rateLimits } from "@repo/drizzle";
...
export class RateLimitService {
  ...
  async check(identifier: string, config: RateLimitConfig): Promise<RateLimitResult> {
    ...
    const [result] = await this.db
      ...
      .from(rateLimits)
      .where(and(eq(rateLimits.key, key), gt(rateLimits.consumedAt, windowStart)));
  ...
  /**
   * Check rate limit and throw an HTTP 429 exception if exceeded.
   */
   ...
   */
  async cleanup(olderThanSeconds = 86400): Promise<void> {
    const cutoff = new Date(Date.now() - olderThanSeconds * 1000);
    await this.db.delete(rateLimits).where(sql`${rateLimits.consumedAt} < ${cutoff}`);
  }
}
