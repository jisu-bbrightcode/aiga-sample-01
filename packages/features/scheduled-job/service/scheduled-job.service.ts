...
export class ScheduledJobService {
  ...
  async getJobRuns(jobId: string, input: { page: number; limit: number }) {
    ...
    const [data, totalResult] = await Promise.all([
      ...
      this.db.select({ count: count() }).from(systemJobRuns).where(eq(systemJobRuns.jobId, jobId)),
    ]);
...
// ============================================================================
// Pagination Helper (inlined from @repo/shared/utils/offset-pagination)
// ============================================================================

function buildPaginatedResult<T>(data: T[], total: number, page: number, limit: number) {
  return {
    data,
    total,
    page,
