 ...
 *
 * Import changed: @repo/core/nestjs/auth -> @repo/core/auth
 */
...
export class ScheduledJobController {
  ...
  @ApiResponse({ status: 200, description: "잡 목록 반환", type: ScheduledJobResponseDto, isArray: true })
  ...
  @ApiResponse({ status: 200, description: "잡 실행 이력 반환 (페이지네이션)", type: JobRunsPageResponseDto })
  @ApiResponse({ status: 401, description: "인증 필요" })
  @ApiResponse({ status: 403, description: "관리자 권한 필요" })
  async getJobRuns(
  ...
  @ApiParam({ name: "jobId", description: "잡 UUID" })
  @ApiResponse({ status: 200, description: "토글된 잡 정보 반환", type: ScheduledJobResponseDto })
  @ApiResponse({ status: 401, description: "인증 필요" })
  @ApiResponse({ status: 403, description: "관리자 권한 필요" })
  @ApiResponse({ status: 404, description: "잡을 찾을 수 없음" })
  async toggleJob(@Param("jobId", ParseUUIDPipe) jobId: string) {
  ...
  @ApiResponse({ status: 200, description: "잡 실행 성공", type: RunJobNowResponseDto })
  @ApiResponse({ status: 400, description: "알 수 없는 잡 키" })
  @ApiResponse({ status: 401, description: "인증 필요" })
  @ApiResponse({ status: 403, description: "관리자 권한 필요" })
  async runJobNow(@Body() body: { jobKey: string }) {
    switch (body.jobKey) {
