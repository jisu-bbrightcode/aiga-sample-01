...
import type { BroadcastInput, UpdateSettingsInput } from "../dto";
import {
...
export class NotificationController {
  ...
  @ApiQuery({ name: "type", required: false, type: String, description: "알림 유형 필터" })
  @ApiResponse({ status: 200, description: "알림 목록 반환 (페이지네이션)", type: NotificationListResponseDto })
  ...
  @ApiResponse({ status: 200, description: '읽지 않은 알림 수 반환', type: UnreadCountResponseDto })
  @ApiResponse({ status: 401, description: '인증 필요' })
  ...
  @ApiResponse({ status: 200, description: "읽음 처리 성공", type: SuccessResponseDto })
  @ApiResponse({ status: 401, description: "인증 필요" })
  ...
  @ApiResponse({ status: 200, description: '전체 읽음 처리 성공', type: MarkAllReadResponseDto })
  @ApiResponse({ status: 401, description: '인증 필요' })
  ...
  @ApiResponse({ status: 200, description: '알림 설정 목록 반환', type: NotificationSettingResponseDto, isArray: true })
  @ApiResponse({ status: 401, description: '인증 필요' })
  ...
  @ApiResponse({ status: 200, description: "설정 업데이트 성공", type: SuccessResponseDto })
  @ApiResponse({ status: 401, description: "인증 필요" })
  @ApiBody({
    schema: {
      type: "object",
  ...
  async updateSettings(@Body() dto: UpdateSettingsInput, @CurrentUser() user: User) {
  ...
  @UseGuards(BetterAuthAdminGuard)
  @ApiOperation({ summary: '[Admin] 전체 공지 발송' })
  @ApiResponse({ status: 201, description: '공지 발송 성공', type: BroadcastResponseDto })
  @ApiResponse({ status: 401, description: '인증 필요' })
  ...
  @ApiResponse({ status: 200, description: "알림 통계 반환", type: NotificationStatsResponseDto })
  @ApiResponse({ status: 401, description: "인증 필요" })
  @ApiResponse({ status: 403, description: "관리자 권한 필요" })
  async getStats() {
