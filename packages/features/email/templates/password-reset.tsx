...
import type { PasswordResetVariables } from "../../common/types";
import { EmailLayout } from "./layout";

/**
 * 비밀번호 재설정 템플릿
 */
export function PasswordResetEmail({ userName, resetUrl, expiresIn }: PasswordResetVariables) {
  return (
    <EmailLayout>
      ...
      <Text style={warningBox}>
        <strong>보안 안내:</strong> 이 링크는 {expiresIn} 동안만 유효합니다. 비밀번호 재설정을
        요청하지 않으셨다면 즉시 계정 보안을 확인하세요.
      </Text>

      <Text style={footnote}>
        비밀번호 재설정을 요청하지 않으셨다면 이 이메일을 무시하셔도 됩니다. 계정은 안전하게
        보호됩니다.
      </Text>
    </EmailLayout>
  );
