...
- `is_deleted`/`deleted_at` 컬럼을 `service_search_documents`에 추가(마이그레이션 0055).
...
마이그레이션 `0055_service_search_archive.sql` (idempotent, hand-authored):
...
(소프트 삭제 후 공개 hot path). PG16에서 0000–0055 from-scratch + 0055 재적용 멱등 검증.
