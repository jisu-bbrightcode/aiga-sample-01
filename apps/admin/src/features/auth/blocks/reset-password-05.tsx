...
import { z } from "zod";
import { AuthLayout05 } from "./shared/auth-layout-05";

const PASSWORD_REGEX = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^\w\s]).{8,16}$/;

interface ResetPassword05Props {
  email: string;
  messages: {
...
export function ResetPassword05({
  email,
  messages,
  onSubmit,
  loading = false,
}: ResetPassword05Props) {
  ...
  return (
    <AuthLayout05 title="Reset Password" description={`Enter your new password for ${email}`}>
