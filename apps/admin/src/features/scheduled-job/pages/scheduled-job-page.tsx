...
import { Sheet, SheetContent, SheetHeader, SheetTitle } from "@repo/ui/shadcn/sheet";
...
const SCHEDULED_JOB_ERROR_MESSAGE = "요청을 처리하지 못했습니다. 잠시 후 다시 시도해 주세요.";
...
export function ScheduledJobPage() {
  ...
  const { data: runsData, isLoading: isRunsLoading } = useJobRuns(selectedJobId, historyPage);
  ...
  const handleToggle = (jobId: string) => {
    toggleJob.mutate(
      ...
      {
        onSuccess: (updated) => {
          toast.success(updated.isActive ? "잡이 활성화되었습니다." : "잡이 비활성화되었습니다.");
        },
        onError: () => {
          toast.error(SCHEDULED_JOB_ERROR_MESSAGE);
        },
      },
    );
  };

  const handleRunNow = (jobKey: ScheduledJobKey, displayName: string) => {
    runJobNow.mutate(
      { jobKey },
      {
        onSuccess: () => {
  ...
  return (
    <div className="container mx-auto py-8">
      <PageHeader title="스케줄러" description="예약 작업을 관리합니다" />
            ...
            <TableBody>
              {jobs.map((job) => (
                <TableRow key={job.id}>
                  ...
                  <TableCell>
                    <span className="text-sm text-muted-foreground">
                      {job.nextRunAt ? new Date(job.nextRunAt).toLocaleString("ko-KR") : "-"}
                    </span>
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => handleRunNow(job.jobKey as ScheduledJobKey, job.displayName)}
        ...
        <SheetContent side="right" className="sm:max-w-lg">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {isRunsLoading ? (
              ...
              <div className="py-12 text-center text-muted-foreground">실행 이력이 없습니다.</div>
            )}
          </div>
        </SheetContent>
      </Sheet>
...
const DEFAULT_STATUS_CONFIG: RunStatusConfig = { label: "실행 중", variant: "default" };

const STATUS_CONFIG: Record<string, RunStatusConfig> = {
  running: DEFAULT_STATUS_CONFIG,
  success: {
    label: "성공",
    variant: "secondary",
