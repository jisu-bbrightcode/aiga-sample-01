...
import { PostCard, type CommunityPostView } from "../components/post-card";
import {
  useCommunity,
  useCommunityPosts,
  useJoinCommunity,
...
export function CommunityHome({ slug }: CommunityHomeProps) {
  ...
  const posts =
    (postsData?.pages.flatMap((page: { items: unknown[] }) => page.items) ??
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="space-y-3">
          {isLoadingPosts
            ...
            : posts.map((post) => <PostCard key={post.id} post={post} communitySlug={slug} />)}
        </div>
