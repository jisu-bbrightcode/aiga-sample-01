      ...
      <Card>
        ...
        <CardContent className="pt-6 space-y-4">
          {isLoadingComments ? (
            ...
            <>
              {commentsData
                .filter((c: { parentId?: unknown; [k: string]: unknown }) => !c.parentId)
                .map((comment: { id: string; parentId?: unknown; [k: string]: unknown }) => (
                  <CommentItem
                    key={comment.id}
                    comment={comment as any}
                    replies={commentsData.filter((c: { parentId?: unknown; [k: string]: unknown }) => c.parentId === comment.id) as any}
                  />
                ))}
              {hasNextPage && (
