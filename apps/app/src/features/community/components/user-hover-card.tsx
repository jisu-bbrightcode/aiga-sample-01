...
export function UserHoverCard({ userId: _userId, username, karma, children }: UserHoverCardProps) {
  ...
  return (
    <Popover>
      <PopoverTrigger openOnHover delay={300} render={<span className="cursor-pointer" />}>
        {children}
      </PopoverTrigger>
      <PopoverContent side="top" sideOffset={8} className="w-56 p-3">
        ...
        <div className="space-y-1 text-xs">
          ...
          <div className="text-muted-foreground flex justify-between">
            ...
            <span className="text-foreground font-medium">
              {karma.commentKarma.toLocaleString()}
            </span>
          </div>
        </div>
      </PopoverContent>
